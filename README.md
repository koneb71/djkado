...
| Area | Highlights |
|---|---|
| **Decks** (2 ↔ 4, animated morph) | Jog wheels (vinyl scratch + pitch bend, inertia), scrolling & overview waveforms (zoom, drag-scrub, beat/bar grid, cue & loop markers), CUE / Play / Sync (tempo + continuous phase lock) / Key lock / Slip / Quantize / Censor / Brake / Backspin, **motor simulation** (pause spins the platter down and play winds it back up, VirtualDJ-style — time adjustable, or off), pitch fader (±8/16/50 %), **performance pads** (8 hot cues · loop roll 1/16–8 · 8-slice slicer with slip · beat jump ±1–16), auto-loop 1/32–64 beats, loop in/out/reloop, key shift ± semitones |
